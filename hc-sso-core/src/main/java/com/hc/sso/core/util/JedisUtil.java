...
public class JedisUtil {
    ...
    /**
     * redis address, like "{ip}"、"{ip}:{port}"、"{redis/rediss}://hc-sso:{password}@{ip}:{port:6379}/{db}"；Multiple "," separated
     */
    ...
    private static ShardedJedis getInstance() {
                ...
                if (INSTANCE_INIT_LOCL.tryLock(2, TimeUnit.SECONDS)) {
                    ...
                    try {
                        ...
                        if (shardedJedisPool == null) {
                            ...
                            logger.info(">>>>>>>>>>> hc-sso, JedisUtil.ShardedJedisPool init success.");
                        }
        ...
        if (shardedJedisPool == null) {
            throw new NullPointerException(">>>>>>>>>>> hc-sso, JedisUtil.ShardedJedisPool is null.");
        }
